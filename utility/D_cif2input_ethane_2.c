...
int main(int argc, char *argv[])
{
	...
	fprintf(fp2,"cutoff(A) FH_signal Mass(g/mol) Tempearture(K)  Running_steps\n");
	fprintf(fp2,"%lf\t%d\t%lf\t%lf\t3000\n", cutoff, FH_signal, mass_molecule, Temperature);
	fprintf(fp2,"---------String Calculation Settings---------\n");
	int direction = 2;
	// int direction = 1;
	int set_N;
	double set_delta_frac;
	double set_delta_angle_degree;
	int order_of_magnitude;
	fprintf(fp2,"Direction\n");
	fprintf(fp2,"%d\n", direction);
	fprintf(fp2,"#_of_points delta_frac delta_angle_degree\n");
	switch (direction)
	{
		case 1:
			set_N = ceil(a/0.1);
	...
	while (1)
	{
		...
		if ((1.0/set_N/2*pow(10,order_of_magnitude))>1)
		{
			break;
		}
		else
	...
	set_delta_frac = pow(10,-(order_of_magnitude+1));
	set_delta_angle_degree = 1;
	fprintf(fp2,"%d %lf %lf\n", set_N, set_delta_frac, set_delta_angle_degree);
	fprintf(fp2,"#_convergence_translational_(%%) #_convergence_rotational_(%%)\n");
	double set_conv_trans, set_conv_rot;
	set_conv_trans = 20;
	set_conv_rot = 20;
	fprintf(fp2,"%lf %lf\n", set_conv_trans, set_conv_rot);
	fprintf(fp2,"------------------Adsorbate------------------\n");
	fprintf(fp2,"Number of sites\n");
	fprintf(fp2,"2\n");
	fprintf(fp2,"x(A)	y(A)	z(A)	Epsilon(K)	Sigma(A)	Mass(g/mol)\n");
	fprintf(fp2,"0.00 0.00 0.00 108 3.76 15\n");
	fprintf(fp2,"0.00 0.00 1.54 108 3.76 15\n");
