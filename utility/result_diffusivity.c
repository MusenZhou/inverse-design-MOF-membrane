...
int main(int argc, char *argv[])
{
	...
	double Tstart = 300, Tend = 300, Tinterval = 1, T;
	// double Tstart = 300, Tend = 300, Tinterval = 1, T;
	// file I/O variable
	FILE *fp1;
	int buffersize = 512;
	...
	fgets(str, buffersize, fp1);
	fscanf(fp1, "%lf %lf %lf", &rand_a, &rand_b, &mass_g_mol);
	// printf("mass: %lf\n", mass_g_mol);
	fgets(str, buffersize, fp1);
	fgets(str, buffersize, fp1);
	fgets(str, buffersize, fp1);
	...
	for (i=0; i<N; i++)
	{
		...
		// printf("%.3e %.3e %.3e\n", string_x[i], string_y[i], string_z[i]);
	}
	...
	double k, D0;
	for (T=Tstart; T<=Tend; T = T + Tinterval)
	{
		for (i=0; i<N; i++)
		{
			if ((v[i]/T)>6e5)
